 ...
 * Fixed: Persistent Change Logging, Detail Modal Sizing, and Header Layout.
 */
...
async function exportData() {
    ...
    if (currentView.history && currentView.history.length > 0) {
        currentView.history.forEach(l => {
            log += `Row #${l.row}, Old: ${l.old}, Updated: ${l.new}\n`;
        });
    } else { log += "No changes recorded in history."; }
...
function openEditor(idx) {
    ...
    const renderControls = (id) => {
        const b = currentView.boxes[id];
        if (!b.isVar) return `<input type="text" value="${b.textVal}" oninput="updateBoxValue(${id}, this.value)" style="width:100%; padding:12px; border-radius:8px; border:1px solid #ddd; box-sizing:border-box;">`;
        if (!currentView.headers || currentView.headers.length === 0) return `<button class="orange-btn" style="width:100%" onclick="uploadExcelFromEditor(${id})">Upload Excel to see Variables</button>`;
        return `<div class="pills-container">${currentView.headers.map(h => `<div class="var-pill ${b.textVal === h ? 'selected' : ''}" onclick="updateBoxValue(${id}, '${h}')">${h}</div>`).join('')}</div>`;
    };
    overlay.innerHTML = `<div class="editor-window"><div class="editor-preview-area"><input type="text" value="${box.title}" oninput="currentView.boxes[${idx}].title=this.value; refreshUI(${idx})" style="font-size:1.5rem; text-align:center; font-weight:bold; border:none; background:transparent; border-bottom:2px solid var(--orange); outline:none; margin-bottom:10px; width:80%;"><div id="prev" class="box-instance" style="--box-w:${box.w}; --box-h:${box.h}; background:${box.bgColor}; color:${box.textColor}; border-radius:15px; position:relative;"><div class="box-title" style="font-size:28px;">${box.title}</div><div id="prev-txt" class="box-content" style="font-size:${box.fontSize}px;">${box.isVar ? '<'+box.textVal+'>' : box.textVal}</div></div></div><div class="editor-controls-area"><div class="property-group"><h4>Background</h4><div class="color-grid">${bgPresets.map(c => `<div class="circle" style="background:${c}" onclick="applyAttr(${idx},'bgColor','${c}')"></div>`).join('')}</div><h4 style="margin-top:15px;">Text Color</h4><div class="color-grid">${textPresets.map(c => `<div class="circle" style="background:${c}" onclick="applyAttr(${idx},'textColor','${c}')"></div>`).join('')}</div></div><div class="property-group"><h4>Font Size</h4><div class="font-controls"><button class="blue-btn" onclick="adjustFont(${idx},-2)">-</button><span id="sz" style="font-size:1.4rem; font-weight:bold;">${box.fontSize}</span><button class="blue-btn" onclick="adjustFont(${idx},2)">+</button></div></div><div class="property-group"><h4>Content Source</h4><div class="segmented-control"><button class="segment-btn ${!box.isVar ? 'active' : ''}" onclick="setMode(${idx},false)">Constant</button><button class="segment-btn ${box.isVar ? 'active' : ''}" onclick="setMode(${idx},true)">Variable</button></div><div id="pills-root">${renderControls(idx)}</div></div><button class="primary-btn" onclick="closePop(); drawBoxes();">Save Box</button></div></div>`;
    document.body.appendChild(overlay);
}

function updateBoxValue(idx, val) {
    currentView.boxes[idx].textVal = val; refreshUI(idx);
    const root = document.getElementById('pills-root');
    if (root) {
        const box = currentView.boxes[idx];
        if (!box.isVar) root.innerHTML = `<input type="text" value="${box.textVal}" oninput="updateBoxValue(${idx}, this.value)" style="width:100%; padding:12px; border-radius:8px; border:1px solid #ddd; box-sizing:border-box;">`;
        else root.innerHTML = `<div class="pills-container">${currentView.headers.map(h => `<div class="var-pill ${box.textVal === h ? 'selected' : ''}" onclick="updateBoxValue(${idx}, '${h}')">${h}</div>`).join('')}</div>`;
    }
}
