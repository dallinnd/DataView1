/**
 * DATA VIEW - MASTER ENGINE v21.0
 * Features: Modern Gradients, Working Pill Layout, Rounded Corners, Drag & Drop.
 */
...
const bgPresets = [
    '#ffffff', '#f1f5f9', '#1e293b',
    'linear-gradient(135deg, #FF5F6D 0%, #FFC371 100%)', // Sunset
    'linear-gradient(135deg, #2193b0 0%, #6dd5ed 100%)', // Ocean
    'linear-gradient(135deg, #ee9ca7 0%, #ffdde1 100%)', // Rose
    'linear-gradient(135deg, #11998e 0%, #38ef7d 100%)', // Lush
    'linear-gradient(135deg, #8E2DE2 0%, #4A00E0 100%)', // Royal
    'linear-gradient(135deg, #00c6ff 0%, #0072ff 100%)', // Deep Blue
    'linear-gradient(135deg, #f97316 0%, #ed8936 100%)', // BYU Orange
    'linear-gradient(135deg, #6366f1 0%, #a855f7 100%)'  // Indigo
];
const textPresets = ['#000000', '#ffffff', '#475569', '#ef4444', '#3b82f6', '#10b981', '#f97316'];
...
function openEditor(idx) {
    ...
    overlay.innerHTML = `<div class="editor-window">
        <div class="editor-preview-area">
            ...
            <div id="prev" class="box-instance" style="--box-w:${box.w}; --box-h:${box.h}; background:${box.bgColor}; color:${box.textColor}; border-radius:15px; position:relative;">
                <div class="box-title" style="font-size:28px;">${box.title}</div>
        ...
        <div class="editor-controls-area">
            ...
            <div class="property-group">
                ...
                <div class="font-controls">
                    <button class="font-btn" onclick="adjustFont(${idx},-2)">-</button>
                    <span id="sz" style="font-size:1.4rem; font-weight:bold;">${box.fontSize}</span>
                    <button class="font-btn" onclick="adjustFont(${idx},2)">+</button>
                </div>
            </div>
            <div class="property-group">
...
function renderContentControls(idx) {
    const box = currentView.boxes[idx];
    if (!box.isVar) {
        return `<input type="text" value="${box.textVal}" oninput="updateBoxValue(${idx}, this.value)" style="width:100%; padding:12px; border-radius:8px; border:1px solid #ddd; box-sizing:border-box;">`;
    }
    ...
    return `<div class="pills-container">
        ${currentView.headers.map(h => `
            <div class="var-pill ${box.textVal === h ? 'selected' : ''}" onclick="updateBoxValue(${idx}, '${h}')">${h}</div>
        `).join('')}
    </div>`;
}
...
function updateBoxValue(idx, val) {
    ...
    refreshUI(idx);
    // Refresh the specific controls div to update "selected" class on pills
    const container = document.getElementById('ctrls-container');
    if (container) container.innerHTML = renderContentControls(idx);
}
    ...
    window.addEventListener('mouseup', (e) => {
        ...
        if (gridX >= 0 && gridY >= 0 && gridX + w <= 6 && gridY + h <= 4) {
            if (isDraggingNew) currentView.boxes.push({ x: gridX, y: gridY, w, h, title: 'Title', textVal: 'Value', isVar: false, bgColor: '#ffffff', textColor: '#000', fontSize: 36 });
                ...
                if (gridX === box.x && gridY === box.y) openChoiceMenu(dragIdx);
                else { box.x = gridX; box.y = gridY; }
            }
            saveAll();
        }
...
function startDragExisting(e, idx) {
    e.preventDefault(); dragIdx = idx; const box = currentView.boxes[idx]; const original = e.currentTarget; const rect = original.getBoundingClientRect(); const containerRect = document.getElementById('canvas-container').getBoundingClientRect();
...
function startDragNew(e, w, h) {
    e.preventDefault(); const container = document.getElementById('canvas-container'); const rect = container.getBoundingClientRect(); const btnRect = e.currentTarget.getBoundingClientRect();
    const cellW = container.offsetWidth / 6; const cellH = container.offsetHeight / 4;
    draggingElement = document.createElement('div'); draggingElement.className = 'box-instance dragging'; draggingElement.style.width = `${cellW * w}px`; draggingElement.style.height = `${cellH * h}px`; draggingElement.style.background = '#3b82f6'; draggingElement.style.color = 'white'; draggingElement.innerHTML = `<div class="box-content">Place in Grid</div>`;
...
function editLiveValue(idx) { const box = currentView.boxes[idx]; const old = currentView.data[currentRowIndex][box.textVal]; const newVal = prompt(`Update ${box.textVal}:`, old); if (newVal !== null && newVal !== old) { changeLog.push({ time: new Date().toLocaleTimeString(), row: currentRowIndex+1, col: box.textVal, old, new: newVal }); currentView.data[currentRowIndex][box.textVal] = newVal; saveAll(); closePop(); renderSlide(); } }
