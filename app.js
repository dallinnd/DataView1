/**
 * DATA VIEW - COMPLETE UNIFIED ENGINE
 * Features: 6x4 Grid, Collision Detection, Excel Integration, 
 * Proportional Editor, Presentation Mode, and Change Logging.
 */
...
document.addEventListener('DOMContentLoaded', () => {
    const saved = localStorage.getItem('dataView_master_v1');
    if (saved) views = JSON.parse(saved);
    renderHome();
});

function saveAll() {
    localStorage.setItem('dataView_master_v1', JSON.stringify(views));
}
...
function createNewView() {
    const nv = { 
        name: 'New View', createdAt: Date.now(), updatedAt: Date.now(), 
        boxes: [], headers: [], data: [], excelName: null 
    };
    views.push(nv);
    currentView = nv;
    renderEditCanvas();
    ...
    for (let i = 0; i < 24; i++) {
        ...
        cell.onclick = () => {
            ...
            currentView.boxes.push({
                ...
                title:'Title', textVal:'Text', isVar:false, 
...
function startPresentation() {
    if (!currentView.data || currentView.data.length === 0) return alert("Upload Excel data first!");
    currentRowIndex = 0;
    renderSlide();
}
...
function nextSlide() { if(currentRowIndex < currentView.data.length - 1) { currentRowIndex++; renderSlide(); } else { alert("Last slide."); renderHome(); } }
...
// --- DATA HELPERS ---
function updateBoxValue(idx, val) { currentView.boxes[idx].textVal = val; refreshUI(idx); if(currentView.boxes[idx].isVar) { closePop(); openEditor(idx); } }
function refreshUI(idx) {
    ...
    if(p) { p.style.background = box.bgColor; p.style.color = box.textColor; t.innerText = box.isVar ? `<${box.textVal}>` : box.textVal; t.style.fontSize = box.fontSize + 'px'; p.querySelector('small').innerText = box.title; }
    saveAll();
}
...
function deleteView(id) { if(confirm("Delete?")) { views=views.filter(v=>v.createdAt!=id); saveAll(); renderHome(); } }
