/**
 * DATA VIEW - MASTER ENGINE v26.0
 * Repaired Initialization, Persistent State, and ZIP Export with Log.
 */
...
async function exportData() {
    ...
    const zip = new JSZip();
    const baseName = (currentView.name || 'View').replace(/\s+/g, '_');
...
function startDragNew(e, w, h) {
    ...
    draggingElement = document.createElement('div'); draggingElement.className = 'box-instance dragging'; draggingElement.style.width = `${cellW * w}px`; draggingElement.style.height = `${cellH * h}px`; draggingElement.style.background = 'var(--primary)'; draggingElement.innerHTML = `<div class="box-content" style="color:white">Drop on Grid</div>`;
...
function openEditor(idx) {
    ...
    const renderControls = () => {
        if (!box.isVar) return `<input type="text" value="${box.textVal}" oninput="updateBoxValue(${idx}, this.value)" style="width:100%; padding:14px; border-radius:10px; border:1px solid #ddd; box-sizing:border-box;">`;
...
function updateBoxValue(idx, val) { currentView.boxes[idx].textVal = val; refreshUI(idx); closePop(); openEditor(idx); }
