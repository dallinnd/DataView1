/**
 * DATA VIEW - MASTER ENGINE v23.0
 * Repaired: Click vs Drag menu conflict, Header positioning, Persistent history.log.
 */
...
async function exportData() {
    ...
    } else { logText += "No changes recorded."; }
...
function handleMouseUp(e) {
    ...
    if (dist < 5 && !isDraggingNew) { // Treat as click
...
function startDragExisting(e, idx) {
    ...
    draggingElement.style.width = `${rect.width}px`; draggingElement.style.height = `${rect.height}px`;
...
function openEditor(idx) {
    ...
    overlay.innerHTML = `<div class="editor-window"><div class="editor-preview-area"><input type="text" value="${box.title}" oninput="currentView.boxes[${idx}].title=this.value; refreshUI(${idx})" style="font-size:1.5rem; text-align:center; font-weight:bold; border:none; background:transparent; border-bottom:2px solid var(--orange); outline:none; margin-bottom:10px; width:80%;"><div id="prev" class="box-instance" style="--box-w:${box.w}; --box-h:${box.h}; background:${box.bgColor}; color:${box.textColor}; position:relative;"><div class="box-title" style="font-size:28px;">${box.title}</div><div id="prev-txt" class="box-content" style="font-size:${box.fontSize}px;">${box.isVar ? '<'+box.textVal+'>' : box.textVal}</div></div></div><div class="editor-controls-area"><div class="property-group"><h4>Background</h4><div class="color-grid">${bgPresets.map(c => `<div class="circle" style="background:${c}" onclick="applyAttr(${idx},'bgColor','${c}')"></div>`).join('')}</div><h4 style="margin-top:15px;">Text Color</h4><div class="color-grid">${textPresets.map(c => `<div class="circle" style="background:${c}" onclick="applyAttr(${idx},'textColor','${c}')"></div>`).join('')}</div></div><div class="property-group"><h4>Font Size</h4><div class="font-controls" style="display:flex; gap:10px; align-items:center;"><button class="blue-btn" onclick="adjustFont(${idx},-2)">-</button><span id="sz" style="font-size:1.4rem; font-weight:bold;">${box.fontSize}</span><button class="blue-btn" onclick="adjustFont(${idx},2)">+</button></div></div><div class="property-group"><h4>Content</h4><div style="display:flex; background:#f1f5f9; padding:4px; border-radius:10px; margin-bottom:10px;"><button style="flex:1; padding:8px; border-radius:8px; background:${!box.isVar?'white':'transparent'}; color:${!box.isVar? 'var(--orange)': 'var(--slate)'}" onclick="setMode(${idx},false)">Constant</button><button style="flex:1; padding:8px; border-radius:8px; background:${box.isVar?'white':'transparent'}; color:${box.isVar? 'var(--orange)': 'var(--slate)'}" onclick="setMode(${idx},true)">Variable</button></div><div id="pills-root">${renderControls()}</div></div><button class="primary-btn" onclick="closePop(); drawBoxes();">Save Box</button></div></div>`;
...
// --- SHARED UTILS ---
function updateBoxValue(idx, val) { currentView.boxes[idx].textVal = val; refreshUI(idx); openEditor(idx); }
