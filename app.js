 ...
 * Fixed: Click vs Drag menu, Top-Right buttons, Pill selection refresh.
 */
...
let dragIdx = -1;
let dragStartX, dragStartY; // To detect if it was a click or a move
let offset = { x: 0, y: 0 };
...
function handleMouseUp(e) {
    ...
    // Distance check: If moved less than 5px, treat as a click
    ...
    if (dist < 5 && !isDraggingNew) {
        ...
        if (isDraggingNew) {
            currentView.boxes.push({ x: gridX, y: gridY, w, h, title: 'Title', textVal: 'Value', isVar: false, bgColor: '#ffffff', textColor: '#000', fontSize: 36 });
        } else {
...
function startDragNew(e, w, h) {
    ...
    draggingElement.innerHTML = `<div class="box-content" style="color:white">Place on Grid</div>`;
...
function openEditor(idx) {
    ...
    overlay.innerHTML = `<div class="editor-window">
        <div class="editor-preview-area"><input type="text" value="${box.title}" oninput="currentView.boxes[${idx}].title=this.value; refreshUI(${idx})" style="font-size:1.5rem; text-align:center; font-weight:bold; border:none; background:transparent; border-bottom:2px solid var(--orange); outline:none; margin-bottom:10px; width:80%;"><div id="prev" class="box-instance" style="--box-w:${box.w}; --box-h:${box.h}; background:${box.bgColor}; color:${box.textColor}; position:relative;"><div class="box-title" style="font-size:28px;">${box.title}</div><div id="prev-txt" class="box-content" style="font-size:${box.fontSize}px;">${box.isVar ? '<'+box.textVal+'>' : box.textVal}</div></div></div>
        <div class="editor-controls-area">
            <div class="property-group"><h4>Background</h4><div class="color-grid">${bgPresets.map(c => `<div class="circle" style="background:${c}" onclick="applyAttr(${idx},'bgColor','${c}')"></div>`).join('')}</div><h4 style="margin-top:15px;">Text Color</h4><div class="color-grid">${textPresets.map(c => `<div class="circle" style="background:${c}" onclick="applyAttr(${idx},'textColor','${c}')"></div>`).join('')}</div></div>
            <div class="property-group"><h4>Font Size</h4><div class="font-controls"><button class="blue-btn" onclick="adjustFont(${idx},-2)">-</button><span id="sz" style="font-size:1.4rem; font-weight:bold;">${box.fontSize}</span><button class="blue-btn" onclick="adjustFont(${idx},2)">+</button></div></div>
            <div class="property-group"><h4>Content</h4><div class="segmented-control"><button class="segment-btn ${!box.isVar ? 'active' : ''}" onclick="setMode(${idx},false)">Constant</button><button class="segment-btn ${box.isVar ? 'active' : ''}" onclick="setMode(${idx},true)">Variable</button></div><div id="pills-root">${renderPills(idx)}</div></div>
...
function updateBoxValue(idx, val) {
    ...
    if (root) root.innerHTML = renderPills(idx);
}

// --- BOILERPLATE & EXPORT ---
async function exportData() {
    if (!currentView || !currentView.data) return alert("No data!");
