/**
 * DATA VIEW - MASTER ENGINE v24.0
 * Fixed: Missing Color Selection, Small Preview Box, and Top-Right Buttons.
 */
...
const iconHome = `<svg viewBox="0 0 24 24" width="22" height="22" fill="white"><path d="M10 20v-6h4v6h5v-8h3L12 3 2 12h3v8z"/></svg>`;
const iconLeft = `<svg viewBox="0 0 24 24" width="22" height="22" fill="white"><path d="M15.41 7.41L14 6l-6 6 6 6 1.41-1.41L10.83 12z"/></svg>`;
const iconRight = `<svg viewBox="0 0 24 24" width="22" height="22" fill="white"><path d="M10 6L8.59 7.41 13.17 12l-4.58 4.59L10 18l6-6z"/></svg>`;

const bgPresets = ['#ffffff','#f1f5f9','#1e293b','linear-gradient(135deg, #FF5F6D 0%, #FFC371 100%)','linear-gradient(135deg, #2193b0 0%, #6dd5ed 100%)','linear-gradient(135deg, #ee9ca7 0%, #ffdde1 100%)','linear-gradient(135deg, #11998e 0%, #38ef7d 100%)','linear-gradient(135deg, #8E2DE2 0%, #4A00E0 100%)','linear-gradient(135deg, #00c6ff 0%, #0072ff 100%)','linear-gradient(135deg, #f97316 0%, #ed8936 100%)'];
...
function openEditor(idx) {
    ...
    const renderControls = () => {
        if (!box.isVar) return `<input type="text" value="${box.textVal}" oninput="updateBoxValue(${idx}, this.value)" style="width:100%; padding:14px; border-radius:10px; border:1px solid #ddd; box-sizing:border-box; font-size:1rem;">`;
    ...
    <div class="editor-window">
        <div class="editor-preview-area">
            <input type="text" value="${box.title}" oninput="currentView.boxes[${idx}].title=this.value; refreshUI(${idx})" style="font-size:1.8rem; text-align:center; font-weight:bold; border:none; background:transparent; border-bottom:3px solid var(--orange); outline:none; margin-bottom:15px; width:70%;">
        ...
        <div class="editor-controls-area">
            <div class="property-group">
                ...
                <h4 style="margin-top:20px;">Text Color</h4>
            ...
            <div class="property-group">
                <h4>Font Size</h4>
                <div class="font-controls" style="display:flex; gap:15px; align-items:center;">
                    <button class="blue-btn" style="width:50px; height:50px; font-size:1.5rem;" onclick="adjustFont(${idx},-2)">-</button>
                    <span id="sz" style="font-size:1.6rem; font-weight:bold; min-width:40px; text-align:center;">${box.fontSize}</span>
                    <button class="blue-btn" style="width:50px; height:50px; font-size:1.5rem;" onclick="adjustFont(${idx},2)">+</button>
                </div>
            </div>
            <div class="property-group">
                <h4>Content Source</h4>
                <div style="display:flex; background:#f1f5f9; padding:5px; border-radius:12px; margin-bottom:15px;">
                    <button style="flex:1; padding:10px; border-radius:10px; background:${!box.isVar?'white':'transparent'}; color:${!box.isVar?'var(--orange)':'var(--slate)'}; font-weight:bold;" onclick="setMode(${idx},false)">Constant</button>
                    <button style="flex:1; padding:10px; border-radius:10px; background:${box.isVar?'white':'transparent'}; color:${box.isVar?'var(--orange)':'var(--slate)'}; font-weight:bold;" onclick="setMode(${idx},true)">Variable</button>
                </div>
                <div id="ctrls-root">${renderControls()}</div>
            </div>
            <button class="primary-btn" onclick="closePop(); drawBoxes();" style="margin-top:10px;">Save Changes</button>
        </div>
    </div>`;
...
function updateBoxValue(idx, val) { 
    ...
    refreshUI(idx); 
    // Re-render the editor to show selection
...
function refreshUI(idx) { 
    ...
    if(p && t) { 
        p.style.background = b.bgColor; p.style.color = b.textColor; 
        t.innerText = b.isVar ? `<${b.textVal}>` : b.textVal; 
        t.style.fontSize = b.fontSize + 'px'; 
        const ti = p.querySelector('.box-title'); if(ti) ti.innerText = b.title; 
    } 
    saveAll(); 
}
...
function handleMouseUp(e) {
    ...
    if (dist < 5 && !isDraggingNew) { 
        draggingElement.remove(); openChoiceMenu(dragIdx); 
...
function startDragNew(e, w, h) {
    ...
    draggingElement = document.createElement('div'); draggingElement.className = 'box-instance dragging'; draggingElement.style.width = `${cellW * w}px`; draggingElement.style.height = `${cellH * h}px`; draggingElement.style.background = 'var(--primary)'; draggingElement.innerHTML = `<div class="box-content" style="color:white">Place on Grid</div>`;
...
async function exportData() {
    ...
    const zip = new JSZip();
    const baseName = currentView.name.replace(/\s+/g, '_');
    const ws = XLSX.utils.json_to_sheet(currentView.data);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "UpdatedData");
    zip.file(`${baseName}_updated.xlsx`, XLSX.write(wb, { bookType: 'xlsx', type: 'array' }));
    let log = `HISTORY LOG - ${currentView.name}\nExported: ${new Date().toLocaleString()}\n----------------\n`;
    if (currentView.history && currentView.history.length > 0) {
        currentView.history.forEach(l => log += `Row #${l.row}, Old: ${l.old}, Updated: ${l.new}\n`);
    } else { log += "No changes recorded."; }
...
function renderSlide() {
    ...
    currentView.boxes.forEach((box, i) => {
        ...
        div.onclick = () => openDetailModal(i, val); canvas.appendChild(div);
    });
}
