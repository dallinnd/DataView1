/**
 * DATA VIEW - MASTER ENGINE v29.0
 * Repaired: Overlap-safe Editor, Click-vs-Drag handling, Persistent history.log.
 */
...
function openEditor(idx) {
    ...
    <div class="editor-window">
        <div class="editor-preview-area">
            <input type="text" id="edit-title" value="${box.title}" oninput="updateTitle(${idx}, this.value)" placeholder="Edit Title..." style="font-size:1.8rem; text-align:center; font-weight:bold; border:none; background:transparent; border-bottom:3px solid var(--orange); outline:none; margin-bottom:15px; width:70%;">
        ...
        <div class="editor-controls-area">
            ...
            <div class="property-group">
                ...
                <div id="pills-root">${renderPills(idx)}</div>
            </div>
...
async function exportData() {
    ...
    const zip = new JSZip(); const fileNameBase = currentView.name.replace(/\s+/g, '_');
    zip.file(`updated_data.xlsx`, XLSX.write({SheetNames:["Data"], Sheets:{"Data":XLSX.utils.json_to_sheet(currentView.data)}}, {bookType:'xlsx', type:'array'}));
    let log = "HISTORY LOG\n"; if (currentView.history) currentView.history.forEach(l => log += `Row #${l.row} | Old: ${l.old} | New: ${l.new}\n`);
    zip.file("history.log", log);
    const content = await zip.generateAsync({ type: "blob" });
    const link = document.createElement('a'); link.href = URL.createObjectURL(content); link.download = `${currentView.name}_Package.zip`; link.click();
}
