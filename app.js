/**
 * DATA VIEW - MASTER ENGINE v11.0
 * Features: Fixed 28px Top Titles, Centered Variable Content, 36pt Default Font,
 * Tab-Isolated Presentation, Auto-Hide SVG Controls, and Keyboard Navigation.
 */
...
function renderHome() {
    ...
    views.sort((a,b) => b.updatedAt - a.updatedAt).forEach(v => {
        ...
        div.innerHTML = `<div><strong>${v.name}</strong></div>
...
function drawBoxes() {
    ...
    currentView.boxes.forEach((box, i) => {
        ...
        div.innerHTML = `
            ...
            <div class="box-content" style="font-size:${box.fontSize}px;">${display}</div>
        `;
...
function renderSlide() {
    ...
    currentView.boxes.forEach((box) => {
        ...
        `;
        div.onclick = () => openDetailModal(views.indexOf(currentView), val);
...
function refreshUI(idx) {
    ...
    if(p && t) {
        ...
        const titleEl = p.querySelector('.box-title'); if(titleEl) titleEl.innerText = box.title;
    }
    saveAll();
}
function uploadExcel() {
    ...
    input.onchange = (e) => {
        const reader = new FileReader(); reader.onload = (evt) => {
            ...
            currentView.headers = Object.keys(data[0] || {}); currentView.data = data; 
