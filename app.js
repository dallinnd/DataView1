/**
 * DATA VIEW PRO - MASTER ENGINE v55.1
 * Enhanced: Cross-Tab Sync & Direct Presentation Export
 */
...
// Sync data across tabs (Presentation Tab <-> Main Tab)
window.addEventListener('storage', (e) => {
    if (e.key === 'dataView_master_v55') {
        loadFromStorage();
        if (currentView) {
            currentView = views.find(v => v.createdAt == currentView.createdAt);
            // Refresh current UI state
            if (document.querySelector('.presentation-fullscreen')) renderSlideContent();
            else if (document.getElementById('view-list')) renderHome();
            else if (document.getElementById('sidebar')) renderEditCanvas();
        }
    }
});
...
function drawBoxes() {
    ...
    currentView.boxes.forEach((box, i) => {
        ...
        div.innerHTML = `<div class="box-title" style="color:${box.textColor || 'black'};">${box.title}</div><div class="box-content" style="font-size:${box.fontSize}px;">${box.isVar ? "Variable" : box.textVal}</div>`;
    ...
    document.getElementById('app').innerHTML = `
        <div class="presentation-fullscreen">
            ...
            <div class="presentation-nav">
                <button onclick="window.close()" title="Home">${iconHome}</button>
                <button onclick="exportFinalFiles()" title="Download Updated Excel">${iconDownload}</button>
...
function exportFinalFiles() {
    if (!currentView || !currentView.data || !currentView.data.length) return alert("No data to export.");
    
    // 1. Export Excel
    const wb = XLSX.utils.book_new();
    const ws = XLSX.utils.json_to_sheet(currentView.data);
    XLSX.utils.book_append_sheet(wb, ws, "Updated Data");
    XLSX.writeFile(wb, `${currentView.name}_Updated_2026.xlsx`);
    ...
    if (currentView.history && currentView.history.length > 0) {
        ...
        a.download = `${currentView.name}_change_log.txt`; 
        a.click();
    }
}
...
function nextSlide() { if(currentRowIndex < currentView.data.length - 1) { currentRowIndex++; updateNavCount(); renderSlideContent(); } }
function prevSlide() { if(currentRowIndex > 0) { currentRowIndex--; updateNavCount(); renderSlideContent(); } }
function updateNavCount() { const span = document.querySelector('.presentation-nav span'); if(span) span.innerText = `${currentRowIndex+1} / ${currentView.data.length}`; }
