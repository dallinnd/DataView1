/**
 * DATA VIEW - MASTER ENGINE v18.0
 * Features: 
 * - Drag & Drop Layout (New & Existing Boxes)
 * - Boundary Detection (Red Warning)
 * - ZIP Export (Excel + history.log)
 * - Live Presentation Editing & Change Logging
...
function renderHome() {
    ...
    app.innerHTML = `
        ...
        <h2 style="text-align:center; margin-top:40px; color:#475569; font-size:1rem; text-transform:uppercase; letter-spacing:0.1em;">Existing Displays</h2>
...
function openMenu(id) {
    ...
    document.getElementById('app').innerHTML = `
        ...
        <div style="display:grid; grid-template-columns:1fr 1fr; gap:20px; margin-top:20px;">
            <button class="blue-btn" style="height:140px; font-size:1.2rem;" onclick="exportData()">Export (.zip)</button>
            <button class="blue-btn" style="height:140px; font-size:1.2rem;" onclick="window.open(window.location.origin+window.location.pathname+'?view=${id}','_blank')">View Presentation</button>
            <button class="blue-btn" style="height:140px; font-size:1.2rem;" onclick="renderEditCanvas()">Edit Layout</button>
            <button class="blue-btn" style="height:140px; font-size:1.2rem; background:var(--danger);" onclick="deleteView('${id}')">Delete View</button>
        </div>
        <button onclick="renderHome()" style="margin-top:30px; width:100%; background:none; text-decoration:underline; border:none; color:var(--slate);">Back to Home</button>
    `;
}
...
function initDragListeners() {
    ...
    window.addEventListener('mouseup', (e) => {
        ...
        if (gridX >= 0 && gridY >= 0 && gridX + w <= 6 && gridY + h <= 4) {
            if (isDraggingNew) {
                currentView.boxes.push({ x: gridX, y: gridY, w, h, title: 'New Box', textVal: 'Value', isVar: false, bgColor: '#ffffff', textColor: '#000', fontSize: 36 });
            } else {
                const idx = draggingElement.getAttribute('data-idx');
                currentView.boxes[idx].x = gridX;
                currentView.boxes[idx].y = gridY;
            }
            saveAll();
        }
...
function startDragExisting(e, idx) {
    ...
    draggingElement.setAttribute('data-idx', idx);
...
function startDragNew(e, w, h) {
    ...
    draggingElement.innerHTML = `<div class="box-content">New ${w}x${h}</div>`;
...
function renderEditCanvas() {
    document.getElementById('app').innerHTML = `
        ...
        <div style="text-align:center; margin-top:15px;"><small color="var(--slate)">Drag a size below onto the grid</small></div>
        <div style="display:flex; justify-content:center; gap:10px; margin-top:10px; flex-wrap:wrap;">
            ${['2x2','2x1','4x1','6x1','3x3','4x4'].map(s => `<button class="blue-btn" style="background:#64748b" onmousedown="startDragNew(event, ${s.split('x')[0]}, ${s.split('x')[1]})">${s}</button>`).join('')}
        </div>`;
    drawGrid(); drawBoxes(); initDragListeners();
}

function drawGrid() {
    const grid = document.getElementById('grid'); grid.innerHTML = '';
    for (let i = 0; i < 24; i++) {
        const cell = document.createElement('div'); cell.className = 'grid-cell'; grid.appendChild(cell);
    }
}
...
function drawBoxes() {
    ...
    currentView.boxes.forEach((box, i) => {
        ...
        div.onmousedown = (e) => { if (e.detail === 1) startDragExisting(e, i); };
...
async function exportData() {
    ...
    zip.file(`${baseName}_edited.xlsx`, excelOut);
    let log = "HISTORY LOG\n================\n";
